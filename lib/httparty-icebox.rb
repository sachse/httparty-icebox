  ...
  module Icebox
    ...
    module Store
      ...
      class FileStore < AbstractStore
        ...
        def set(key, value)
          Cache.logger.info("Cache: set (#{key})")
          File.open( @path.join(key), 'w' ) { |file| file << ActiveSupport::Base64.encode64(Marshal.dump(value))  }
          true
        end
        def get(key)
          data = Marshal.load(ActiveSupport::Base64.decode64(File.read( @path.join(key))))
          Cache.logger.info("Cache: #{data.nil? ? "miss" : "hit"} (#{key})")
          data
        end
        def exists?(key)
